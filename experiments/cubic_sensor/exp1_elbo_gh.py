...
def main():
    ...
    # ours: damped hybrid-markov smoother (FH, GSLR), one curve per trust-region eps
    for family in ["FH", "GSLR"]:
        log_prior_fn, log_trans_fn, log_obsrv_fn = cs.make_model_fns(
            system, family, cs.GH5_QUAD if family == "FH" else cs.GH5_LINEAR
        )
    ...
    # IPLS: undamped native GH5 statistical-linear-regression + RTS, re-linearized each iteration
    transition, observation = cs.make_additive_models(system)

    def ipls_step(forward, _):
        m = std_forward_message(forward)
        lin_t = jax.vmap(lambda q: AffineGaussian(*cs.GH5_LINEAR(transition, q)))(Gaussian(m.mean[:-1], m.cov[:-1]))
        lin_o = jax.vmap(lambda q: AffineGaussian(*cs.GH5_LINEAR(observation, q)))(Gaussian(m.mean[1:], m.cov[1:]))
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "fig_cubic_elbo_nlpd_gh.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    for which, fname, (lo, hi) in [(0, "results_cubic_elbo_gh.csv", elbo_ylim), (1, "results_cubic_nlpd_gh.csv", nlpd_ylim)]:
