"""Cubic sensor (exp3): cubature + forward-markov smoother + init_temperature=1e8 (paper-era FH/GSLR recipe); ELBO evaluated with GH5."""
...
def main():
    ...
    print(f"cubic sensor: T={num_steps}, seed={data_seed}, eps={kl_steps}, {max_iter} iters, init_temp={init_temp:g}")

    results = {}

    # ours: damped forward-markov smoother (FH, GSLR) with cubature, one curve per trust-region eps
    for family in ["FH", "GSLR"]:
        ...
        for step in kl_steps:
            ...
            elbo = np.asarray(jax.vmap(elbo_of)(h["marginals"], h["kernels"]))[keep]
            nlpd = np.asarray(jax.vmap(nlpd_of)(h["marginals"]))[keep]
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "fig_cubic_elbo_nlpd_fwd.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    for which, fname, (lo, hi) in [(0, "results_cubic_elbo_fwd.csv", elbo_ylim), (1, "results_cubic_nlpd_fwd.csv", nlpd_ylim)]:
